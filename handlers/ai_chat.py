...
async def hexi_group_trigger(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """وقتی داخل گروه با «Hexi»/«هگزی» صداش بزنن (متن)، یا مستقیم روی یکی از
    پیام‌های خودِ Hexi ریپلای کنن (بدون نیاز به تکرار اسمش)، جواب می‌ده. برای
    پیام‌هایی که هیچ‌کدوم از این دوتا نباشن، بی‌صدا هیچ کاری نمی‌کنه (نه تماس
    با AI، نه هزینه)."""
    ...
    is_reply_to_hexi = bool(reply_to and reply_to.from_user and reply_to.from_user.id == context.bot.id)
